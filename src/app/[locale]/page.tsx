...
import {Link} from "@/navigation"; // Use from next-intl
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from "@/components/ui/card";
import { CheckCircle, ScanLine, MessageCircle, CookingPot, BarChart3 as ChartColumn, Users, Target, Lightbulb } from "lucide-react"; // Renamed BarChart3
  ...
  const features = [
    {
      ...
      title: "Smart Label Analysis", // This text would also be translated in a full i18n setup
        ...
        <div className="container px-4 md:px-6 text-center">
          ...
          <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
            <Button asChild size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground transition-transform duration-300 ease-in-out hover:scale-105">
              <Link href="/analyze">{t('analyzeLabelButton')}</Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="transition-transform duration-300 ease-in-out hover:scale-105 border-primary text-primary hover:bg-primary/5">
              <Link href="/recipes">{t('getRecipeIdeasButton')}</Link>
            </Button>
          </div>
        </div>
      </section>
        ...
        <div className="container px-4 md:px-6">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">
              Unlock a Healthier You {/* Placeholder - To be translated */}
            </h2>
            <p className="mx-auto max-w-[700px] text-muted-foreground md:text-xl mt-4">
              Our AI-powered tools provide comprehensive insights for informed food choices. {/* Placeholder */}
            </p>
          </div>
        ...
        <div className="container grid items-center gap-8 px-4 md:px-6 lg:grid-cols-2 lg:gap-16">
          ...
          <div>
            ...
            <p className="mt-4 max-w-[600px] text-muted-foreground md:text-xl/relaxed">
              To empower every Indian with the knowledge and tools to make healthier food choices, demystifying nutrition labels and promoting traditional healthy eating habits through accessible AI technology. {/* Placeholder */}
            </p>
            <ul className="mt-6 space-y-4">
              <li className="flex items-start">
                <CheckCircle className="mr-3 mt-1 h-5 w-5 flex-shrink-0 text-accent" />
                <div>
                  <h3 className="font-semibold">Simplify Nutrition</h3> {/* Placeholder */}
                  <p className="text-sm text-muted-foreground">
                    Break down complex food information into easy-to-understand insights. {/* Placeholder */}
                  </p>
                </div>
              </li>
              <li className="flex items-start">
                <CheckCircle className="mr-3 mt-1 h-5 w-5 flex-shrink-0 text-accent" />
                <div>
                  <h3 className="font-semibold">Promote Healthy Alternatives</h3> {/* Placeholder */}
                  <p className="text-sm text-muted-foreground">
                    Highlight nutritious Indian food options and recipes. {/* Placeholder */}
                  </p>
                </div>
              </li>
              <li className="flex items-start">
                <CheckCircle className="mr-3 mt-1 h-5 w-5 flex-shrink-0 text-accent" />
                <div>
                  <h3 className="font-semibold">Foster Awareness</h3> {/* Placeholder */}
                  <p className="text-sm text-muted-foreground">
                    Educate users about hidden ingredients and making informed choices. {/* Placeholder */}
                  </p>
                </div>
              </li>
            </ul>
             <Button asChild size="lg" className="mt-8 bg-accent hover:bg-accent/90 text-accent-foreground transition-transform duration-300 ease-in-out hover:scale-105">
                <Link href="/blogs">Read Our Blog</Link> {/* Placeholder */}
            </Button>
          </div>
        </div>
      </section>
      ...
      <section id="testimonials" className="w-full py-12 md:py-24 lg:py-32">
        <div className="container px-4 md:px-6">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">
              Loved by Users Across India {/* Placeholder */}
            </h2>
            <p className="mx-auto max-w-[700px] text-muted-foreground md:text-xl mt-4">
              Hear what our community is saying about Swasth Bharat Advisor. {/* Placeholder */}
            </p>
          </div>
      ...
      <section id="how-it-works" className="w-full py-12 md:py-24 lg:py-32 bg-muted/40">
        <div className="container grid items-center gap-8 px-4 md:px-6 lg:grid-cols-2 lg:gap-16">
          <div>
            ...
            <p className="mt-4 max-w-[600px] text-muted-foreground md:text-xl/relaxed">
              Our platform uses cutting-edge AI to simplify nutrition for you. {/* Placeholder */}
            </p>
            <ul className="mt-6 space-y-4">
              <li className="flex items-start">
                ...
                <div>
                  <h3 className="font-semibold">Input Your Data</h3> {/* Placeholder */}
                  <p className="text-sm text-muted-foreground">
                    Upload a food label image, or manually enter ingredients and nutritional facts. {/* Placeholder */}
                  </p>
                </div>
              </li>
              <li className="flex items-start">
                ...
                <div>
                  <h3 className="font-semibold">AI Analysis</h3> {/* Placeholder */}
                  <p className="text-sm text-muted-foreground">
                    Our advanced AI models extract text, analyze for health insights, ratings, and alternatives. {/* Placeholder */}
                  </p>
                </div>
              </li>
              <li className="flex items-start">
                ...
                <div>
                  <h3 className="font-semibold">Get Smart Advice</h3> {/* Placeholder */}
                  <p className="text-sm text-muted-foreground">
                    Receive a comprehensive report, chat with our AI for clarifications, or get recipe ideas. {/* Placeholder */}
                  </p>
                </div>
              </li>
            </ul>
      ...
      <section id="call-to-action" className="w-full py-16 md:py-28 lg:py-36 bg-primary text-primary-foreground">
        <div className="container px-4 md:px-6 text-center">
          <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl mb-6">
            Ready to Make Healthier Choices? {/* Placeholder */}
          </h2>
          <p className="mx-auto max-w-[600px] text-primary-foreground/80 md:text-xl mb-8">
            Join thousands of Indians on their journey to better nutrition. Start analyzing, learning, and cooking with Swasth Bharat Advisor today! {/* Placeholder */}
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Button asChild size="lg" className="bg-background text-primary hover:bg-background/90 transition-transform duration-300 ease-in-out hover:scale-105">
              <Link href="/analyze">Start Analyzing Labels</Link> {/* Placeholder */}
            </Button>
            <Button asChild variant="secondary" size="lg" className="transition-transform duration-300 ease-in-out hover:scale-105">
              <Link href="/nutrition-check">Check Nutrition Facts</Link> {/* Placeholder */}
            </Button>
          </div>
        </div>
      </section>
