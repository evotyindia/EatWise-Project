...
export async function generateMetadata({ params }: BlogPostPageProps) {
  ...
  // Metadata translation would require a different setup, e.g. loading messages here
  return {
...
export default function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  return (
    <article className="container mx-auto max-w-3xl py-8 px-4 md:px-6">
      <div className="mb-8">
        <Button variant="outline" asChild size="sm" className="mb-6 group">
          <Link href="/blogs">
            ...
            Back to Blog {/* Placeholder text */}
          </Link>
        </Button>
        <h1 className="text-4xl font-bold tracking-tight mb-3">{post.title}</h1> {/* Not translated, uses blog data */}
        <div className="flex items-center space-x-4 text-sm text-muted-foreground mb-4">
          <div className="flex items-center">
            <CalendarDays className="mr-1.5 h-4 w-4" />
            <span>{new Date(post.date).toLocaleDateString(params.locale, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
          </div>
          <div className="flex items-center">
            <Tag className="mr-1.5 h-4 w-4" />
            <Badge variant="secondary">{post.category}</Badge> {/* Category not translated */}
          </div>
        </div>
        <Image
      ...
      {/* Blog post content is HTML and not translated by this setup */}
      <div
      ...
      <div className="text-center">
        <Button asChild>
          <Link href="/analyze">Analyze Your Food Label Now</Link> {/* Placeholder text */}
        </Button>
      </div>
    </article>
  );
